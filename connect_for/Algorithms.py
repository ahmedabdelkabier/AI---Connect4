...
class Algorithms:
    ...
    def game_over(current_state):
        return winning(current_state , player_card) or winning(current_state , AI_card) or len(get_valid_location(current_state)) == 0

    def winnig(current_state , card):
        #check horizontal win
        for col in range(colums - 3):
            for r in range(rows):
                if current_state[r][col] == card and current_state[r][col + 1] == card and current_state[r][col + 2] == card and current_state[r][col + 3] == card:
                    return True
        #check columns win
        for col in range(columns):
            for r in range(rows - 3):
                if current_state[r][col] == card and current_state[r + 1][col] == card and current_state[r + 2][col] == card and current_state[r + 3][col] == card:
                    return True

        #check diagonal1
        for col in range(columns - 3):
            for r in range(rows - 3):
                if current_state[r][col] == card and current_state[r + 1][col + 1] == card and current_state[r + 2][col + 2] == card and current_state[r + 3][col + 3] == card:
                    return True
